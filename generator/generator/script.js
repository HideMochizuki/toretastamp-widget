...
function applyCurrentDesignToMock() {
    ...
    // 変数名を再宣言せず、querySelectorで再取得して使う
    const menuSublist = mock.querySelector('.menu-sublist');

    // ★修正1：削除する前に「今、開いているか？」の状態を保存しておく
    const oldBtn = phoneContainer.querySelector('.hamburger-btn');
    const wasOpen = oldBtn && oldBtn.classList.contains('active');
    ...
    if (listPattern === 'B' && menuSublist) {
        ...
        // ★修正2：保存しておいた状態（wasOpen）を復元する
        // これにより、色を変えても「開いたまま」になります
        if (wasOpen) {
            hamBtn.classList.add('active');
            menuSublist.classList.add('open');
            overlay.classList.add('show');
            // 開いている時の色を適用
            spans.forEach(s => s.style.backgroundColor = hamActiveColor);
        } else {
            // 閉じている時の色を適用
            spans.forEach(s => s.style.backgroundColor = hamLineColor);
        }
        ...
        const toggleMenu = (e) => {
            ...
            // ★修正3：ここに以前あった「×印の色調整（白にする処理）」は削除しました。
            // これがあると設定した色が上書きされてしまいます。
        };
